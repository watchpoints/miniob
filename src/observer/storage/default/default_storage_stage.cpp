...
{
  LOG_TRACE("DefaultStorageStage::handle_even Enter\n");
  ...
  {                                                           // create table
    const CreateTable &create_table = sql->sstr.create_table; //创建表语法tree
  ...
  {
    //删除表：
    LOG_INFO("FSCF_DROP_TABLE");
    const DropTable &drop_table = sql->sstr.drop_table; //删除表语法tree
  ...
  case SCF_CREATE_INDEX:
  {
    ...
    { 
      ...
      rc = handler_->create_index_multi(current_trx, current_db, create_index.relation_name,
                               create_index.index_name,create_index.attr_num, create_index.attributes);
    }
    snprintf(response, sizeof(response), "%s\n", rc == RC::SUCCESS ? "SUCCESS" : "FAILURE");
  }
  break;
