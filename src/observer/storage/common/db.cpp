...
RC Db::create_table(const char *table_name, int attribute_count, const AttrInfo *attributes) {
  ...
  ///Users/wangchuanyi/code/logs/miniob/db/sys
  std::string table_file_path = table_meta_file(path_.c_str(), table_name); // 文件路径可以移到Table模块
  LOG_INFO("create table . table_file_path =%s,path =", table_file_path.c_str(),path_.c_str());
  ...
  opened_tables_[table_name] = table; //保存到内存中：
...
RC Db::drop_table(const char *table_name) {
  ...
  // check table_name
  if (opened_tables_.count(table_name) == 0) {
    return RC::SCHEMA_TABLE_NOT_EXIST; //已经不存在了
  }

  std::string table_file_path = table_meta_file(path_.c_str(), table_name); // 文件路径可以移到Table模块
  
  std::string table_data_file_path = table_meta_data_file(path_.c_str(), table_name); 

  //find table_meta_data_file

  Table *table = opened_tables_[table_name];
  rc = table->drop(table_file_path.c_str(), table_name, table_data_file_path.c_str());
  
  if (rc != RC::SUCCESS) {
    LOG_INFO("drop table failed table_file_path name=%s,%s", table_file_path.c_str(),table_data_file_path.c_str());
    return rc;
  }
  ...
  LOG_INFO("drop table success table_file_path name=%s,%s", table_file_path.c_str(),table_data_file_path.c_str());
