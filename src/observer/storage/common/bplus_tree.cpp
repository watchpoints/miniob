...
static int CmpRid(const RID *rid1, const RID *rid2)
{
  ...
  LOG_INFO("CmpRid >>>: rid1(%d,%d),rid2:(%d,%d)", rid1->page_num, rid1->slot_num, rid2->page_num, rid2->slot_num);
...
//比较2个key 大小
int CompareKeyUnique(const char *pdata, const char *pkey, AttrType attr_type, int attr_length)
{ // 简化
  int i1, i2;
  float f1, f2;
  const char *s1, *s2;
  ...
  case INTS:
  {
    i1 = *(int *)pdata;
    i2 = *(int *)pkey;
    LOG_INFO("CompareKeyUnique int >>>: 1=%d,2=%d", i1, i2);
    if (i1 > i2)
      return 1;
    if (i1 < i2)
      return -1;
  ...
  case DATES:
  {
    i1 = *(int *)pdata;
    i2 = *(int *)pkey;
    LOG_INFO("CompareKeyUnique DATES >>>: 1=%d,2=%d", i1, i2);
...
int CmpKeyUnique(AttrType attr_type, int attr_length, const char *pdata, const char *pkey)
{
  ...
  if (0 == result)
  {
    LOG_INFO("CmpKeyUnique  RECORD_DUPLICATE_KEY");
...
RC BplusTreeHandler::insert_entry_unique(const char *pkey, const RID *rid)
{
  if (rid)
  {
    LOG_INFO(" BplusTreeHandler::insert_entry_unique  of record(rid=%d.%d)",
             rid->page_num, rid->slot_num);
  }
  RC rc;
  PageNum leaf_page;
...
RC BplusTreeHandler::find_leaf_unique(const char *pkey, PageNum *leaf_page)
{
  ...
  while (0 == node->is_leaf)
  {
    for (i = 0; i < node->key_num; i++)
    {
      LOG_INFO(" >>>>>>>>find_leaf_unique  find_leaf_unique >>>>>>find_leaf_unique ");
      tmp = CmpKeyUnique(file_header_.attr_type, file_header_.attr_length, pkey, node->keys + i * file_header_.key_length);

      if (tmp == 0)
      {
        LOG_INFO(" find_leaf_unique 索引重复了,RECORD_DUPLICATE_KEY");
        return RC::RECORD_DUPLICATE_KEY;
      }
...
//为了减少影响，这里没有通过函数增加参数方式：
//风险：一次代码变更 需要修改多处。容易遗漏
//标记一次 后面处理
RC BplusTreeHandler::insert_into_leaf_unique(PageNum leaf_page, const char *pkey, const RID *rid)
{
  ...
  for (insert_pos = 0; insert_pos < node->key_num; insert_pos++)
  {
    LOG_INFO(">>>>>>>insert_into_leaf_unique insert_into_leaf_unique ");
    tmp = CmpKeyUnique(file_header_.attr_type, file_header_.attr_length, pkey, node->keys + insert_pos * file_header_.key_length);
    if (tmp == 0)
    {
      LOG_INFO(" 索引重复了,RECORD_DUPLICATE_KEY");
      //rid1(1,15),rid2:(1,11)
